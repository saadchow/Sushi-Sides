...
export default function OrderHistoryPage() {
  ...
  // Hooks must be top-level (unconditional) — moved here.
  const money = useMemo(
    () =>
  ...
  if (!orders.length) {
    return (
      <div className="OrderHistoryPage empty">
        ...
        <Link to="/orders/new" className="Button back">
          Back to Menu
        </Link>
      </div>
    );
        ...
        {orders.map((o) => (
          <li key={o._id} className="OrderCard">
            ...
            <div className="OrderSummary">
              {(o.totalQty ?? 0)} {o.totalQty === 1 ? 'item' : 'items'} —{' '}
              {money.format(o.orderTotal ?? 0)}
            </div>
            {!!o.lineItems?.length && (
              <ul className="LineItems">
                {o.lineItems.map((li) => (
                  <li key={li._id || `${li.item?._id}-${li.qty}`}>
                    {li.qty}× {li.item?.name ?? 'Item'} —{' '}
                    {money.format(li.extPrice ?? li.qty * (li.item?.price ?? 0))}
                  </li>
                ))}
              </ul>
            )}
